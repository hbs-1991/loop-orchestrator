...
| `.loop.yml` format (schema v1) | Fields: `specs_dir`, `base_branch`, `setup`, `run`, `test`, `required_env`, `timeout_minutes`, `sandbox_preset`, `e2e.services`, `e2e.env`. Later phases extend it **additively only**, every addition optional and defaulting to the behaviour that preceded it: `approval`, `review.*`, `e2e.enabled`, `e2e.max_fix_iterations` and `planning.*` (`enabled`, `model`, `advisor.enabled`, `advisor.model`, `advisor.max_iterations`) | The config spreads across every repository the user owns — changing the format is expensive, so it only ever grows and an existing file never needs an edit |
